...
rand <- function(filename)
{
  ...
  num_target <- 6
  num_attr <- ncol(data) - num_target - 1
  print(num_attr)
  ...
  while(target_index < num_target)
  {
    ...
    for(folds_index in 1 : 10)
    {
      ...
      test_y <- test_file[, num_attr + target_index + 1]
      for(each in c(1:length(test_y)))
      {
        if(test_y[each] == 1)
            test_y[each] = 'B'
        else
      ...
      test_x <- test_file[, 1 : num_attr]
      train_y <- train_file[, num_attr + target_index + 1]
      for(each in c(1:length(train_y)))
      {
        if(train_y[each] == 1)
          train_y[each] = 'B'
        else
    ...
    print(mean(error_DT[,1]))
    print(mean(error_DT[,2]))
    print(error_DT)
    # cat("end " + as.character(target_index) + "!#####")
...
# filenames <- c("data\\sample\\labeled\\sample_2days_labeled.csv","data\\sample\\labeled\\feature_chi2sample_2days_labeled.csv","data\\sample\\labeled\\feature_GBCTsample_2days_labeled.csv","data\\sample\\labeled\\feature_L2sample_2days_labeled.csv","data\\sample\\labeled\\feature_RFEsample_2days_labeled.csv")
# filenames <- c("data\\sample\\labeled\\GBDT_0sample_7days_labeled.csv", "data\\sample\\labeled\\GBDT_3sample_7days_labeled.csv")
filenames <- c("data\\sample\\labeled\\GBDT__votesample_7days_labeled.csv")
